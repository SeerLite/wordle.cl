...
export const getWordOfDay = () => {
  ...
  const now = Date.now()
  const msInDay = 86400000 // every 24 hours (24*60*60*1000)
  const today = Math.floor((now - epochMs) / msInDay)
  // Add 787 which is (index_when_we_played_12 - index_if_we_always_played_24) of the afternoon I changed it from a 12-hour to a 24-hour interval
  // Made the change seamless
  const index = 787 + today
